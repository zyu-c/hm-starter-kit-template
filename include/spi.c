...
void init_spi_gyro(void) {
    ...
    *****************************************************************************************/
    ...
    RSPI1.SPPCR.BYTE =
        0x00;  // RSPI通常モード　MOSIの出力値は前回転送の最終データ
    ...
    // RSPIデータコントロールレジスタ設定
    RSPI1.SPDCR.BIT.SPFC =
        0x00;  //使用するフレーム数の設定 一回のやり取りで一つのフレームを格納
    RSPI1.SPDCR.BIT.SPRDTD = 0x00;  // SPDRは受信バッファを読み出す
    RSPI1.SPDCR.BIT.SPLW = 0x01;  // SPDRレジスタへはロングアクセス(SPLE =
                                  // 1の時はロングワードアクセス)

    // RSPCK遅延値の設定
    RSPI1.SPCR.BIT.MSTR =
        0x0;  // RSPCKとSSLNDレジスタの書き込みエラー回避用に設定
    RSPI1.SPCR.BIT.SPE =
        0x0;  // RSPCKとSSLNDレジスタの書き込みエラー回避用に設定
    ...
    //コマンドレジスタの設定
    //シーケンス長の長さ分だけ行う
    RSPI1.SPCMD0.BIT.CPHA =
        0x01;  //偶数エッジでデータサンプル、奇数エッジでデータ変化
    RSPI1.SPCMD0.BIT.CPOL = 0x01;  //アイドル時のRSPCKがHigh
    RSPI1.SPCMD0.BIT.BRDV =
        0x03;  //ビットレート分周をベースのビットレート(ビットレート設定のNの数字=8)を選択
    RSPI1.SPCMD0.BIT.SSLA = 0x00;  // SSL信号アサート設定(SSL0を使う)
    RSPI1.SPCMD0.BIT.SSLKP =
        0x00;  //転送終了後から次アクセス開始までSSL信号レベルを保持
...
int ovre_check(void) { return ovre; }

int gyro_read_check(void) { return read; }

int gyro_write_check(void) { return gyro_write_cnt; }

long read_gyro_data(void) { return gyro_data; }
...
void init_spi_enc(void) {
    ...
    *****************************************************************************************/
    ...
    // RSPIデータコントロールレジスタ設定
    RSPI0.SPDCR.BIT.SPFC =
        0x00;  //使用するフレーム数の設定 一回のやり取りで一つのフレームを格納
    RSPI0.SPDCR.BIT.SPRDTD = 0x00;  // SPDRは受信バッファを読み出す
    RSPI0.SPDCR.BIT.SPLW = 0x00;  // SPDRレジスタへはワードアクセス(SPLE =
                                  // 1の時はロングワードアクセス)

    // RSPCK遅延値の設定
    RSPI0.SPCR.BIT.MSTR =
        0x0;  // SPCKDとSSLNDレジスタの書き込みエラー回避用に設定
    RSPI0.SPCR.BIT.SPE =
        0x0;  // SPCKDとSSLNDレジスタの書き込みエラー回避用に設定
    ...
    //割り込みマスクの設定
    RSPI0.SPCR2.BYTE =
        0x01;  //パリティ機能を有効　偶数パリティ　アイドル割り込み要求を禁止
    ...
    //コマンドレジスタの設定
    //シーケンス長の長さ分だけ行う
    RSPI0.SPCMD0.BIT.CPHA =
        0x01;  //偶数エッジでデータサンプル、奇数エッジでデータ変化
    RSPI0.SPCMD0.BIT.CPOL = 0x01;  //アイドル時のRSPCKがHigh
    RSPI0.SPCMD0.BIT.BRDV =
        0x03;  //ビットレート分周をベースのビットレート(ビットレート設定のNの数字=8)を選択
    RSPI0.SPCMD0.BIT.SSLA = 0x02;  // SSL信号アサート設定(SSL0を使う)(2 SSL2)
    RSPI0.SPCMD0.BIT.SSLKP =
        0x00;  //転送終了後から次アクセス開始までSSL信号レベルをネゲート
    RSPI0.SPCMD0.BIT.SPB =
        0x9;  // RSPIデータ長を10ビットに設定(9~16まで設定可能この設定がエンコーダの分解能となる)
...
int enc_write_cnt_check(void) { return enc_write_cnt; }

int get_enc_data(void) { return enc_r_data; }
