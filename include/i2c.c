...
struct str_IIC {
    unsigned char
        SlvAdr;       /* Slave Address, Don't set bit0. It's a Read/Write bit */
...
void init_I2C(void) {
    ...
    RIIC2.ICCR1.BIT.IICRST = 1;  // IICRST 内部I2Cリセット状態 IICRST=1 &
                                 // ICE=0でRIIC全レジスタリセット
    ...
    RIIC2.ICMR1.BYTE = 0x28;  // bitcounter=9 PCLK/4=10MHz 0.08us
                              // 12M -> 28, 10M -> 38
    RIIC2.ICBRL.BYTE =
        0xF0;  //転送速度=1/{[ICBRH+1)+(ICBRL+1)]/IICφ}	12M -> F0, 10M -> FD
    RIIC2.ICBRH.BYTE = 0xE7;  // IICφ=PCLKx10^6×分周比　Tr=0の場合は400kHz
                              // 12M -> E7, 10M -> F8
    ...
    //	RIIC2.ICIER.BYTE =0xfc;//送信データエンプティ、送信終了割り込み
    //受信データフル割り込み、NACK受信割り込み、ストップコンディション検出割り込み、スタートコンディション検出割り込み
...
void int_i2c_ee(void) {
    ...
    if ((RIIC2.ICSR2.BIT.STOP != 0) && (RIIC2.ICIER.BIT.SPIE != 0)) {
        ...
        iic_buff.trm_cnt =
            0; /* Clear the internal transmission counter for IIC */
    ...
    if ((RIIC2.ICSR2.BIT.START != 0) && (RIIC2.ICIER.BIT.STIE != 0)) {
        ...
        if ((iic_buff.trm_cnt == 0) &&
            (iic_buff.rcv_cnt == 0)) {  //最初のスタートコンディション
            RIIC2.ICDRT = (iic_buff.SlvAdr
                           << 1); /* When master transfer, b0 must be '1' */
        } else {
            RIIC2.ICDRT = (iic_buff.SlvAdr << 1) |
                          0x01; /* When master transfer, b0 must be '1' */
        }
    }
}
...
void int_i2c_tx(void) {
    ...
    if (iic_buff.trm_cnt == 1) {
        ...
        //		if(GYRO_ACCE_F==0){
        //			RIIC2.ICDRT = 0xac;//out_z_L ??read
        //最上位アドレスが1の場合、アドレスがauto incr 		}else{
        // RIIC2.ICDRT = 0xaa;//out_y_L ??read
        //		}
    } else if (iic_buff.trm_cnt ==
               (iic_buff.RWCnt + 2)) {  //書き込みデータ終わり
        if (iic_buff.RW == 0) {
            RIIC2.ICIER.BIT.TEIE = 1; /* Enable Transmit End Interrupt */
        }
    }
...
void int_i2c_te(void) {
    if (iic_buff.RW == 0) {  // writeのみ
        ...
        RIIC2.ICIER.BIT.STIE =
            1; /* Enable Start Condition Detection Interrupt */
